...
public class HearPositionCon : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        var frontPosition = selfPosition;
        var yAdjustMaxDistance = 1.5f; // 補正可能な高さ 
        var rayOrigin = frontPosition + (Vector2.up * yAdjustMaxDistance); // Rayの発射位置
                                                                           // 自身の前方の少し上から真下に向けてRayを飛ばす
        var castResult = Physics2D.Raycast(rayOrigin, Vector2.down, distance: yAdjustMaxDistance);
        Debug.DrawRay((Vector2)selfPosition, Vector2.down * yAdjustMaxDistance, Color.red);
